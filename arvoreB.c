...
void InsereArvoreB(ArvoreB *arvB, int k, int t) { // r = raiz; k = elemento a ser inserido; t = termo de limitação
    NoArvB *r = arvB->raiz;
...
void ImprimeArvoreB(NoArvB *r, int nivel, int t) {
    ...
    // Imprimir a indenta��o baseada no n�vel atual (inversamente proporcional ao n�vel)
